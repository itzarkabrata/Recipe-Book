...
const SearchBox = ({searchShow}:PropsType) => {
    ...
    return (
        <AnimatePresence>
            <motion.form initial={{opacity : 0}} animate={{opacity : 1}} exit={{opacity : 0}} className={`${searchShow?"z-30":"hidden z-20"}  top-40 absolute left-0 w-full py-10 flex justify-center overflow-hidden`} onSubmit={handleSubmit}>
