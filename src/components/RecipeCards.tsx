...
export default function RecipeCards({dataobj}: Props) {
  ...
  return (
    <div>
        ...
        <div className="flex max-sm:flex-col justify-between items-center gap-5 w-full max-sm:h-48 h-44 p-2 sm:p-5 rounded-xl cursor-pointer" style={bgStyle} onClick={handleRrecipePopUp}>
            <div id="recipe-image" className=" max-sm:h-[60%] max-sm:w-[90%] sm:min-w-40 rounded-xl shadow-black shadow-2xl max-h-[96px] max-w-[168px]">
                <img className="h-[96px] w-[168px] rounded-xl" src={dataobj.image_url} alt="image"/>
            </div>
